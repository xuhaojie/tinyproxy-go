...
func handle(client net.Conn) {
	...
	endPos := bytes.IndexByte(b[:], '\n')
	if endPos < 0 || endPos >= n {
		log.Println(errors.New("bad request"))
		return
	}

	var method, URL, address string
	// 从客户端数据读入 method，url
	fmt.Sscanf(string(b[:endPos]), "%s%s", &method, &URL)
	...
	if method == "CONNECT" {
		...
		if !strings.Contains(hostPortURL.Host, ":") { //host 不带端口， 默认 80
			...
			address = fmt.Sprintf("%s:%d", hostPortURL.Host, hostPortURL.Port)
		}
	}

	log.Printf("%v -> %s %s\n", client.RemoteAddr(), method, address)
